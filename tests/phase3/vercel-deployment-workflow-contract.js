...
assert.match(workflow, /MISSING_FOUNDER_READ_API/);
assert.match(workflow, /MISSING_APPS_SCRIPT_READER/);
assert.match(workflow, /MISSING_DOMAIN_CORE/);
...
const bundleIndex = workflow.indexOf('Bundle Founder API into isolated function artifact');
...
assert.ok(testsIndex >= 0 && bundleIndex > testsIndex, 'Tests must run before building the deployment artifact.');
assert.ok(candidateIndex > bundleIndex, 'Founder API must be bundled before creating a candidate deployment.');
...
assert.ok(vercelConfig.functions['api/founder.js']);
